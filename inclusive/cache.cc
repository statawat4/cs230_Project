...
void back_invalidate(uint8_t cache_type,uint64_t victim_address, uint8_t valid, uint32_t cpuNum, uint64_t instId){
  if(valid == 1){
    if (cache_type ==  IS_L1D || cache_type ==  IS_L1I){
        ooo_cpu[cpuNum].L1D.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,false);
        ooo_cpu[cpuNum].L1I.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,false);
    }
    if(cache_type == IS_L2C){
        //bool de4 = false;
        bool de5 = false;
        //de4 = ooo_cpu[cpuNum].L1D.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,false);
        de5 = ooo_cpu[cpuNum].L1I.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,false);
        ooo_cpu[cpuNum].L2C.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,  de5);
    }
      ...
      for(int i=0;i<NUM_CPUS;i++){
            ...
            bool de2 = false;
            bool de3 = false;
            de1 = ooo_cpu[i].L1D.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,false);
            de2 = ooo_cpu[i].L1I.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,false);
            de3 = ooo_cpu[i].L2C.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,de1 || de2);
            uncore.LLC.back_invalidate_entry(cache_type,victim_address,cpuNum,instId,de1 || de2 || de3);
      }
    }
  }
}

bool CACHE::back_invalidate_entry(uint8_t eviction_cache,uint64_t inval_addr,uint32_t fill_cpu, uint64_t instid,bool evicted_from_l1)
{
    ...
    for (uint32_t way=0; way<NUM_WAY; way++) {
        if ((block[set][way].valid == 1 && block[set][way].tag == inval_addr)) {
            if(eviction_cache == IS_L1D || eviction_cache == IS_L1I){
                if(block[set][way].dirty){
                    ...
                    writeback_packet.instr_id = instid;
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            ...
            back_invalidate(cache_type,block[set][way].address, block[set][way].valid,fill_cpu,MSHR.entry[mshr_index].instr_id);
        }
        else if(cache_type == IS_L2C){
