...
#ifdef LLC_BYPASS
    // bypassing if the it is the LLC, no other condition
    if ((cache_type == IS_LLC))
...
#endif
    uint8_t do_fill = 1;
    if (cache_type == IS_L2C) // we writeback regardless of the block being dirty or clean
    { 
      
      // write back to LLC whether dirty or clean
...
#endif
    }else{  //otherwise we check for cleanliness
        // is this dirty?
      ...
      else
      {
        ...
        if(cache_type == IS_LLC && !block[set][way].dirty){
          ...
          if(cache_type == IS_L2C || cache_type == IS_LLC){
            // we do not check if the block is dirty or clean
            ...
            //we go by the default non-inclusive way
              if(block[set][way].dirty){
        ...
        if (do_fill)
        {
          ...
          if (cache_type == IS_LLC)
          {
            ...
            // do nothing here as the block has been invalidated already
          }
          else
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      { // read hit
        ...
        // update replacement policy
        if (cache_type == IS_LLC) // for exclusivity we invalidate the data block in the LLC and do not update the replacement state
        {
